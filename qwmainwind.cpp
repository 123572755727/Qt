...
#include    <QMessageBox>  //用于显示About弹窗
...
{ //信号与槽的关联，当函数带有参数时，必须写明参数的类型
    ...
    //连接About按钮的"点击"信号到对应的槽函数
    connect(ui->mainToolBar->findChild<QAction*>("actAbout"), SIGNAL(triggered()),
            this, SLOT(on_actAbout_triggered()));
}

//About按钮的槽函数（点击按钮时执行，显示包含姓名、学号的弹窗）
void QWMainWind::on_actAbout_triggered()
{
    //自定义弹窗内容
    QString aboutInfo="姓名：林佳怡\n"
                        "学号：2023414290419\n";
    //显示About弹窗
    QMessageBox::about(this, "About", aboutInfo);
}
